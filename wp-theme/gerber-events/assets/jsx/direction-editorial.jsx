/* eslint-disable */
// GERBER EVENTS — Direction Éditoriale (WP build).
// Lit toutes les données depuis la prop `data` (= window.GERBER_DATA injecté par PHP).

const N_TOKENS = {
  dark: {
    bg: '#0a0a0a', bgSoft: '#141414', bgDeep: '#050505',
    paper: '#f4f1ec', fg: '#f4f1ec',
    fgMute: 'rgba(244,241,236,0.70)', fgFaint: 'rgba(244,241,236,0.40)',
    rule: 'rgba(244,241,236,0.16)', ruleStrong: 'rgba(244,241,236,0.32)',
    accent: '#f4f1ec', rowHover: 'rgba(244,241,236,0.04)'
  },
  light: {
    bg: '#ebe5dc', bgSoft: '#dfd9cf', bgDeep: '#d6d0c5',
    paper: '#0a0a0a', fg: '#0a0a0a',
    fgMute: 'rgba(10,10,10,0.62)', fgFaint: 'rgba(10,10,10,0.38)',
    rule: 'rgba(10,10,10,0.20)', ruleStrong: 'rgba(10,10,10,0.36)',
    accent: '#0a0a0a', rowHover: 'rgba(10,10,10,0.04)'
  }
};
...
// Root
// -------------------------------------------------------------------
function DirectionEditorial({ data, tweaks }) {
  const t = tweaks || {};
  ...
  const display = typoStack(t.typo || 'serif', 'display');
  const body    = typoStack(t.typo || 'serif', 'body');
  const script  = typoStack(t.typo || 'serif', 'script');
  const mono    = typoStack(t.typo || 'serif', 'mono');
  ...
  return (
    <div
      className="ge-root"
      data-mode={mode}
      style={{ background: C.bg, color: C.fg, fontFamily: body, fontSize: 15, lineHeight: 1.5 }}>

      <Hero C={C} dt={dt} af={af} display={display} body={body} script={script} mono={mono} D={D} />
      <FilmRoll C={C} dt={dt} af={af} mono={mono} script={script} D={D} />
      <Manifesto C={C} dt={dt} af={af} display={display} script={script} mono={mono} D={D} />
      <KeywordMarquee C={C} mono={mono} af={af} D={D} />
      <Disciplines C={C} dt={dt} af={af} display={display} body={body} script={script} mono={mono} D={D} />
      <ContactStrip C={C} dt={dt} mono={mono} script={script} display={display} D={D} />
      ...
      <style>{`
        ...
        .grain::before {
          ...
          pointer-events:none; animation:ge-grain 8s steps(8) infinite alternate;
        }
        ...
        @media(max-width:767px){
          .ge-hero-stage-grid { grid-template-columns:1fr!important; }
          .ge-hero-sidebar { display:none!important; }
          .ge-filmroll-grid { grid-template-columns:repeat(2,1fr)!important; }
          .ge-disc-header { grid-template-columns:1fr!important; }
          .ge-disc-header > *:last-child { display:none!important; }
          .ge-disc-row { grid-template-columns:60px 72px 1fr!important; }
          .ge-disc-row > *:nth-child(5), .ge-disc-row > *:nth-child(6) { display:none!important; }
          .ge-contact-grid { grid-template-columns:1fr!important; gap:24px!important; }
        }
      `}</style>
    </div>
  );
...
function Hero({ C, dt, af, display, body, script, mono, D }) {
  ...
  return (
    <section
      ...
      className="grain"
      style={{
        position: 'relative', minHeight: 'max(760px, 100vh)',
        overflow: 'hidden', background: C.bgDeep, color: C.fg,
        cursor: 'none', isolation: 'isolate'
      }}>

      <HeroBackdrop C={C} mouse={mouse} af={af} parallax={parallax} bgUrl={bgUrl} />
      <NavBar C={C} mono={mono} script={script} display={display} fade={fade} D={D} />
      <HeroStage C={C} dt={dt} af={af} display={display} body={body} script={script} mono={mono} mouse={mouse} scrollY={scrollY} D={D} />
...
function HeroBackdrop({ C, mouse, af, parallax, bgUrl }) {
  ...
  return (
    <div aria-hidden style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      <div style={{
        ...
        opacity: reveal, maskImage: mask, WebkitMaskImage: mask,
...
function CursorDot({ C, mouse, af, mono }) {
  ...
  return (
    <div aria-hidden style={{
      ...
      width: 1, height: 1, pointerEvents: 'none', zIndex: 30
    }}>
      <div style={{ position:'absolute', left:-22, top:-22, width:44, height:44, borderRadius:'50%', border:`1px solid ${C.fg}`, opacity:.6 }} />
...
function NavBar({ C, mono, script, display, fade, D }) {
  ...
  return (
    <header style={{
      ...
      <a href={home} style={{ display:'flex', alignItems:'center', gap:14, textDecoration:'none', color:C.fg, minWidth:0 }}>
        <GEMark size={32} fg={C.fg} bg={C.bgDeep} />
        <span style={{ display:'flex', flexDirection:'column', lineHeight:1, gap:4, whiteSpace:'nowrap' }}>
          <span style={{ fontFamily:display, fontSize:18, letterSpacing:'.01em', lineHeight:1 }}>GERBER EVENTS</span>
          <span style={{ fontFamily:script, fontSize:17, color:C.fgMute, lineHeight:.85, marginTop:-2 }}>since 1983</span>
        </span>
      </a>
      ...
      <div style={{
        ...
        <span style={{ display:'inline-block', width:5, height:5, background:C.fg, borderRadius:3 }} />
        <a href="#contact" style={{ color:C.fg, textDecoration:'none' }}>Brief · FR/EN</a>
      </div>
    </header>
  );
}
...
function HeroStage({ C, dt, af, display, body, script, mono, mouse, scrollY, D }) {
  const lift = Math.min(80, scrollY * 0.4);
  return (
    <div className="ge-hero-stage-grid" style={{
      ...
      <WordmarkLockup C={C} display={display} script={script} mono={mono} af={af} D={D} />
      <HeroSidebar C={C} display={display} body={body} script={script} mono={mono} af={af} mouse={mouse} D={D} />
    </div>
  );
}

function WordmarkLockup({ C, display, script, mono, af, D }) {
  const size   = 'clamp(96px, 14.5vw, 240px)';
  const hero   = D.hero || {};
  const eyebrow = hero.eyebrow || 'Dossier I · Manifeste · Édition MMXXVI';
  return (
    <div style={{ position:'relative', animation: af>0 ? `ge-rise ${0.9*af}s ${0.05*af}s both` : 'none' }}>
      <div style={{
      ...
      <h1 style={{
        ...
        fontSize:size, lineHeight:0.86, letterSpacing:'-0.02em', color:C.fg,
        textShadow: C.bg==='#0a0a0a' ? '0 1px 0 rgba(0,0,0,.5)' : 'none'
      }}>
        <div style={{ whiteSpace:'nowrap', fontFamily:'"Kanilia","DM Serif Display",serif', fontWeight:400 }}>GERBER</div>
        <div style={{ whiteSpace:'nowrap', fontFamily:'"Kanilia","DM Serif Display",serif', fontWeight:400 }}>EVENTS</div>
        <div style={{
          fontFamily:script, fontWeight:400, fontSize:'0.34em', letterSpacing:'normal',
          color:C.fg, lineHeight:1, marginTop:'0.06em', paddingLeft:'0.6em', whiteSpace:'nowrap'
        }}>since 1983</div>
      </h1>

      <div style={{
        marginTop:28, display:'flex', flexWrap:'wrap', alignItems:'baseline', gap:22,
        fontFamily:mono, fontSize:10.5, letterSpacing:'.18em', textTransform:'uppercase', color:C.fgMute
      }}>
        <span>Atelier Lyon · Genève</span>
        <span style={{ color:C.fgFaint }}>·</span>
        <span>Quatre disciplines · une seule signature</span>
        <span style={{ color:C.fgFaint }}>·</span>
        <span style={{ color:C.fg }}>Brief direct → {(D.contact && D.contact.phone) || '+33 6 00 00 00 00'}</span>
      </div>
    </div>
  );
}

function HeroSidebar({ C, display, body, script, mono, af, mouse, D }) {
  const x     = mouse.active ? mouse.rel.x * 100 : 50;
  const y     = mouse.active ? mouse.rel.y * 100 : 50;
  const hero  = D.hero || {};
  const quote = hero.quote    || 'Concevoir, créer, casser les codes.';
  const scriptTxt = hero.script || 'backstage mindset.';
  const bodyTxt   = hero.body   || 'David Gerber dessine, code, construit et met en lumière — souvent dans le même projet.';
  const projects  = hero.projects || '142+';

  return (
    <aside className="ge-hero-sidebar" style={{
      ...
      <p style={{ margin:0, color:C.fgMute, fontSize:15.5, lineHeight:1.55, maxWidth:460, fontFamily:body }}>
        {bodyTxt}
      </p>
      ...
      <div style={{
        ...
        <div>
          ...
          }} className="ge-num">{projects.replace('+','')}<span style={{ opacity:.6 }}>+</span></div>
        </div>
...
function HeroBaseline({ C, mono, mouse, dt }) {
  return (
    <div style={{
      position:'absolute', bottom:'clamp(16px,2.5vh,32px)',
      left:'clamp(20px,4vw,56px)', right:'clamp(20px,4vw,56px)',
      zIndex:7, display:'flex', alignItems:'center', justifyContent:'space-between',
      fontFamily:mono, fontSize:10.5, letterSpacing:'.18em', textTransform:'uppercase', color:C.fgFaint
    }}>
...
// FILM ROLL
// -------------------------------------------------------------------
function FilmRoll({ C, dt, af, mono, script, D }) {
  const ref   = useRef(null);
  const seen  = useInView(ref);
  const stills = (D.stills && D.stills.length) ? D.stills : [];
  ...
  return (
    <section ref={ref} style={{
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr auto auto', gap:24, alignItems:'baseline', marginBottom:36 }}>
        <div>
          <div style={{ fontFamily:mono, fontSize:11, letterSpacing:'.22em', textTransform:'uppercase', color:C.fgMute, marginBottom:10 }}>
      ...
      <div className="ge-filmroll-grid" style={{ display:'grid', gridTemplateColumns:'repeat(6,minmax(0,1fr))', gap:14 }}>
        {stills.map((s, i) =>
          <FilmFrame key={s.src || i} s={s} i={i} seen={seen} C={C} mono={mono} af={af} />
        )}
      </div>
    </section>
  );
...
function FilmFrame({ s, i, seen, C, mono, af }) {
  ...
  return (
    <figure
      ...
      style={{
        position:'relative', margin:0, aspectRatio:'3/4',
        overflow:'hidden', background:C.bgSoft, border:`1px solid ${C.rule}`, cursor:'pointer',
      ...
      {['tl','tr','bl','br'].map((c) => {
        const sty = { position:'absolute', width:10, height:10, borderColor:C.paper, borderStyle:'solid', borderWidth:0, opacity:hover?1:.55, transition:'opacity .25s' };
      ...
      <figcaption style={{
        ...
        color:C.paper, fontFamily:mono, fontSize:9.5, letterSpacing:'.12em', textTransform:'uppercase',
...
// MANIFESTO
// -------------------------------------------------------------------
function Manifesto({ C, dt, af, display, script, mono, D }) {
  const ref  = useRef(null);
  const seen = useInView(ref);
  const M    = D.manifesto || {};
  const qA   = M.quoteA   || 'Concevoir, créer,';
  const qB   = M.quoteB   || 'casser les codes.';
  const scr  = M.script   || 'backstage mindset.';
  const fn   = M.footnote || 'Atelier Gerber — depuis MCMLXXXIII';
  ...
  return (
    <section ref={ref} style={{
      ...
      <div style={{ maxWidth:1240, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr', gap:36, textAlign:'center' }}>
        <div style={{
        ...
        <blockquote style={{
          ...
          fontSize:'clamp(48px,7.5vw,132px)', lineHeight:0.98, letterSpacing:'-0.025em', color:C.fg,
...
// MARQUEE
// -------------------------------------------------------------------
function KeywordMarquee({ C, mono, af, D }) {
  const raw    = (D.marquee && D.marquee.length) ? D.marquee : [
    'CONCEPTION','DÉVELOPPEMENT','RÉNOVATION','ÉVÉNEMENTIEL',
    'SCÉNOGRAPHIE','SON & LUMIÈRE','BÉTON CIRÉ','IDENTITÉ VISUELLE'
  ];
  const tokens = raw.reduce((a, t) => [...a, t, '×'], []);

  return (
    <div aria-hidden style={{ borderTop:`1px solid ${C.rule}`, borderBottom:`1px solid ${C.rule}`, overflow:'hidden', padding:'20px 0', background:C.bg }}>
      <div style={{
        display:'inline-flex', gap:36, whiteSpace:'nowrap',
        animation: af>0 ? `ge-marquee ${48/(af+0.2)}s linear infinite` : 'none',
        fontFamily:mono, fontSize:12.5, letterSpacing:'.24em', textTransform:'uppercase', color:C.fgMute
      }}>
...
// DISCIPLINES
// -------------------------------------------------------------------
function Disciplines({ C, dt, af, display, body, script, mono, D }) {
  const ref         = useRef(null);
  const seen        = useInView(ref);
  const [active, setActive] = useState(null);
  const services    = (D.disciplines && D.disciplines.length) ? D.disciplines : [];
  const DH          = D.disciplinesHeader || {};
  const eyebrow     = DH.eyebrow || 'III — Disciplines';
  const title       = DH.title   || 'Quatre métiers, un seul atelier — pensés ensemble pour que la fabrique reste continue.';

  return (
    <section ref={ref} style={{ position:'relative', padding:`${dt.sectionPadY} ${dt.pad}`, background:C.bg }}>
      <div className="ge-disc-header" style={{ display:'grid', gridTemplateColumns:'220px 1fr 240px', alignItems:'baseline', gap:40, marginBottom:64 }}>
        <div style={{ fontFamily:mono, fontSize:11, letterSpacing:'.22em', textTransform:'uppercase', color:C.fgMute, display:'flex', alignItems:'center', gap:12 }}>
          <StarMark size={11} color={C.fg} />
          {eyebrow}
        </div>
      ...
      <ol onMouseLeave={() => setActive(null)} style={{ listStyle:'none', padding:0, margin:0 }}>
        {services.map((s, i) =>
          <DisciplineRow key={s.n || i} s={s} i={i} seen={seen}
            active={active===i} anyActive={active!==null}
            onEnter={() => setActive(i)}
            C={C} display={display} body={body} script={script} mono={mono} af={af} total={services.length} />
        )}
      </ol>
    </section>
  );
...
function DisciplineRow({ s, i, seen, active, anyActive, onEnter, C, display, body, script, mono, af, total }) {
  const aff = af===0 ? 0.0001 : af;
  return (
    <li className="ge-disc-row"
      onMouseEnter={onEnter}
      style={{
        position:'relative', display:'grid',
        gridTemplateColumns:'100px 96px 1fr 280px 90px 40px',
        gap:28, alignItems:'center', padding:'24px 12px',
        borderTop:`1px solid ${C.rule}`,
        borderBottom: i===total-1 ? `1px solid ${C.rule}` : 'none',
        cursor:'pointer',
      ...
      <span className="ge-num" style={{
        fontFamily:display, fontSize:'clamp(28px,2.6vw,40px)',
        color: active ? C.fg : C.fgFaint,
        letterSpacing:'-0.02em', lineHeight:1, transition:'color .25s'
      }}>{s.n}</span>

      <div style={{
        position:'relative', width:96, height:120, overflow:'hidden', background:C.bgSoft,
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        <h3 style={{ margin:0, fontFamily:display, fontWeight:400, fontSize:'clamp(26px,2.8vw,44px)', lineHeight:1.05, letterSpacing:'-0.015em', color:C.fg }}>
          {s.title}
        </h3>
        <div style={{
          overflow:'hidden', maxHeight: active ? 80 : 0, opacity: active ? 1 : 0,
          transition:`max-height ${0.35*aff}s var(--ge-ease), opacity ${0.25*aff}s var(--ge-ease)`
        }}>
          <p style={{ margin:'8px 0 0', color:C.fgMute, fontSize:14.5, maxWidth:620, fontFamily:body }}>{s.blurb}</p>
        </div>
        <span style={{
      ...
      <span style={{ fontFamily:mono, fontSize:11, letterSpacing:'.18em', textTransform:'uppercase', color: active ? C.fg : C.fgFaint, textAlign:'right' }}>
        {s.kicker || ''}
      </span>

      <span aria-hidden style={{
        fontFamily:mono, fontSize:20, color: active ? C.fg : C.fgFaint, textAlign:'right',
...
// CONTACT STRIP
// -------------------------------------------------------------------
function ContactStrip({ C, dt, mono, script, display, D }) {
  const contact = D.contact || {};
  const email   = contact.email   || 'david@gerber-events.fr';
  const phone   = contact.phone   || '+33 6 00 00 00 00';
  const address = contact.address || '14 rue des Capucins, Lyon';
  const cta     = contact.cta     || '→ Envoyer un brief';
  const scr     = contact.script  || 'backstage welcome.';
  const year    = (D.site && D.site.year) || new Date().getFullYear();

  return (
    <footer id="contact" style={{
      padding:`clamp(56px,7vh,96px) ${dt.pad} clamp(36px,5vh,64px)`,
      background:C.bgDeep, borderTop:`1px solid ${C.rule}`, color:C.fg
    }}>
      <div className="ge-contact-grid" style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', gap:40, alignItems:'end', marginBottom:40 }}>
        <div>
          <div style={{ fontFamily:mono, fontSize:11, letterSpacing:'.22em', textTransform:'uppercase', color:C.fgMute, marginBottom:16, display:'flex', alignItems:'center', gap:12 }}>
            <StarMark size={11} color={C.fg} />
            Prendre rendez-vous
          </div>
        ...
        <div style={{ alignSelf:'end', fontFamily:mono, fontSize:11, letterSpacing:'.16em', textTransform:'uppercase', color:C.fgMute, lineHeight:1.9, textAlign:'right' }}>
          <div style={{ color:C.fg }}>{email}</div>
          <div>{phone}</div>
          <div>Atelier · {address}</div>
        </div>

        <a href={`mailto:${email}`} style={{
          alignSelf:'end', display:'inline-flex', alignItems:'center', gap:14,
          padding:'16px 24px', border:`1px solid ${C.fg}`,
